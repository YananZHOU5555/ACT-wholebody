...
def main():
    ...
    ckpt_dir = "/home/zeno/NPM-VLA-Project/NPM-VLA/IL_policies/checkpoints/push_block_dp/pretrained_model"
    ...
    rate = rospy.Rate(50)  # Control frequency in Hz

    # Safety parameters
    MAX_JOINT_DELTA = 1  # Maximum joint position change per step (radians)
    ...
    while not rospy.is_shutdown():
        ...
        if out.ndim == 3:
            action = out[0, 0, :]  # first element of the horizon
        else:
            action = out[0, :]

        action = action.cpu().numpy()  # [14]

        # Validate action dimension
        if len(action) != 14:
            rospy.logwarn(
                f"[SAFETY] Invalid action dimension: expected 14, got {len(action)}. Skipping this action."
            )
            rate.sleep()
            continue
        ...
        # Log actions periodically to avoid spam
        rospy.loginfo_throttle(2.0, "✓ Successfully sent actions to robot arms")
