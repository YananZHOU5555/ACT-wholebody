...
def load_policy(model_name: str, checkpoint_dir: str, device: str):
    """Load ACT-wholebody policy from checkpoint."""
...
def main():
    ...
    # Load model
    policy, config = load_policy(args.model, args.checkpoint_dir, device)
    ...
    if config.mix:
        rospy.Subscriber("/robot/base/cmd_vel", Twist, cb_base_vel, queue_size=1)
        rospy.loginfo("Subscribed to base velocity (mix=True)")
    ...
    while not rospy.is_shutdown():
        ...
        state_tensor = torch.from_numpy(state_14).float()

        # Base velocity: 3D (only if mix=True)
        if config.mix and latest_base_vel is not None:
            base_vel_tensor = torch.from_numpy(latest_base_vel).float()
        else:
        ...
        if config.use_torque:
            ...
            effort_tensor = torch.from_numpy(effort_14).float()
        else:
        ...
        action = action_tensor.cpu().numpy()  # (17,): [base_vel(3), arm_joints(14)]
